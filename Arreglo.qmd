...
### ESTE PEDAZO CREA UNA SOLA ISOCRONA

# Configurar la llave de HERE
here_key <- "AQUI SE NECESITA TU LLAVE"
set_key(here_key)

# Crear isocronas
iso <- isoline(
  # Punto(s) desde donde parte la isocrona. Debe ser un objeto sf
  poi = hospitales_centroides[1,],
...
isocronas_sector_censal <- leaflet() %>% 
  ...
  addCircleMarkers(data = hospitales_centroides[1,], 
                   color = "green")
...
# Isocronas para cada hospital

```{r}


###### NO EVALUAR ESTE PEDAZO PORQUE PRODUCE TODAS LAS ISOCRONAS ######

# Crear una carpeta para guardar los shapefiles de las isocronas
dir.create("data/shp/isocronas_hospitales", 
           showWarnings = FALSE)

# Bluce para crear un conjunto de isocronas de 0-10 y 10-20 min por cada centroide
for(i in 1:nrow(hospitales_centroides)){
  
  isocrona <- isoline(
    poi = hospitales_centroides[i,],
  ...
  dsn <- paste0("data/shp/isocronas_hospitales")
  
  # Asignar un nombre a cada archivo shapefile
  layer <- hospitales_centroides$name[i]
...
1.  Trazar los centroides de las zonas censales sobre las isocronas (zonas de influencia).
2.  Generar una matriz que identifique en qué isocrona está localizada cada sector censal.
:::

## Importar datos de sectores censales

Aquí la gente va a necesitar las zonas censales de quito y se va a usar para obtener las geometrías en la sección [Generar la matriz de influencia completa]
...
centroides_dmq <- st_centroid(mapa_base_dmq) %>% 
  ...
  # Seleccionar solo las columnas de nombre y geometría
  select(5,6)
...
st_write(
  centroides_dmq,
  "data/shp/centroide_sector_censal.shp"
)
  
```

## Trazar las ubicaciones de las zonas censales sobre las isocronas de los hospitales (zonas de influencia)

```{r}
# Importamos las isocronas correspodiente al primer hospital  
isocrona_ejemplo_hospital <- st_read("data/shp/isocronas_hospitales/Hospital del Sur (Enrique Garces).shp")

# Hacemos una intersección entre los centroides (puntos) y la isocronas (polígonos)
interseccion <- st_join(centroides_dmq, 
                        isocrona_ejemplo_hospital,
  ...
  # Agrupamos por cada sector censal
  group_by(zon) %>%
  
  # Seleccionar la isocrona más cercana (menor rango de tiempo)
  # evitando duplicados si un mismo centroide cae en varias isocronas del mismo hospital.
  slice_min(order_by = rank, n = 1, with_ties = FALSE) %>%
  ungroup()

```

Visualizamos cómo se verían los sectores censales que caen dentro de las isocronas del primer hospital.

```{r}
# Visualizar los sectores censales que caen dentro de las isocronas
...
Vas a necesitar todas las isocronas para hacer esto, así que voy a subirlas a un drive-

```{r}
#| eval: false

# Ruta donde se encuentran las isocronas
isocronas_lista <- "data/shp/isocronas_hospitales/"
...
lista <- setNames(
  lapply(seq_along(isocrona_ruta), function(i) {
    shapefile <- st_read(isocrona_ruta[i])
    
    st_join(centroides_dmq, shapefile, join = st_within) %>%
      group_by(zon) %>%
      slice_min(order_by = rank, n = 1, with_ties = FALSE) %>%
      ungroup() %>%
      as_tibble()
...
matriz_completa <- map2(shape.names, lista, function(nombre, elemento){
  elemento %>% select(zon, name) %>% 
    rename_with(.fn = ~nombre, .cols = "name")
}) %>% 
  reduce(full_join) 


# Aquí se necesita los sectores censales para obtener las geometrías 
matriz_completa <- left_join(mapa_base_dmq, matriz_completa)

```

# Me quedé desde aquí para abajo
...
peso_NA <- 0
peso_10 <- impedancia(mintiempo = 5, subzonatiempo =  5, maxtiempo = 10)
